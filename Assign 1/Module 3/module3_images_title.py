...
def scrape_news_page(soup: BeautifulSoup) -> List[Dict]:
    ...
    for article in soup.select('article'):
        ...
        if title_elem:
            ...
            # Extract image thumbnail for the article
            image_elem = article.select_one('img')
            image_url = image_elem.get('src') if image_elem else None
            
            # Append article data with headline and image URL
            articles.append({
                'title': title_elem.get_text(strip=True),
                'url': full_url,
                'image_url': image_url  # Thumbnail URL
            })
    return articles
...
def save_output(data: List[Dict], format: str = 'json'):
    ...
    if format == 'json':
        with open('./images_titles.json', 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
    else:
        with open('./images_titles.csv', 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=data[0].keys())
            writer.writeheader()
            writer.writerows(data)
    print(f"Successfully saved {len(data)} articles to images_titles.{format}")
...
def main():
    ...
    output_format = args.format or config.get('output_format', 'json')
